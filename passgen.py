...
def derive_password(master, website, allowed_chars, length):
    hash_bytes = hashlib.pbkdf2_hmac('sha256', bytes(master, 'utf-8'),
                                     bytes(website + str(length), 'utf-8'),
                                     100000)
    # TODO: Use itertools to pretty this up
    password = ''
    i = 0
...
parser.add_argument('--allowed', default=DEFAULT_ALLOWED_CHARS,
                    help='list of allowed characters.')
parser.add_argument('--length', default=DEFAULT_PASSWORD_LENGTH,
                    type=int, help='password length.')
parser.add_argument('website')

args = parser.parse_args()
master_password = getpass.getpass(prompt='Master password:')
print(derive_password(master_password, args.website, args.allowed,
                      args.length))
